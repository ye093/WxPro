...
function doUpload(filePath, success, fail, options) {
    ...
    //自定义属性 add by yejinyun
    formData['x:objectid'] = config.customeObjectId;
    formData['x:type'] = config.customeType;
...
function getQiniuToken(callback) {
  ye.request({
    ...
    success: function (res) {
      //add by yejinyun: customeObjectId and customeType
      var customeObjectId = res.data.objectid;
      if (customeObjectId && customeObjectId.length > 0) {
        config.customeObjectId = customeObjectId;
      }

      var customeType = res.data.type;
      if (customeType && customeType.length > 0) {
        config.customeType = customeType;
      }
